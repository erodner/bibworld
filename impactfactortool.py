...
parser.add_argument(
    "-f", "--factors", help="JSON file with impact factors", required=True
)
...
parser.add_argument(
    "-n", "--notesbib", help="Output bibtex file with notes fields", default=None
)
parser.add_argument(
    "-w", "--wordforip", help="Word used for impact factor", default="impact factor"
)
            ...
            if not identifier is None:
                if re.search(f, identifier):
                    ...
                    if not args.notesbib is None:
                        notesbib.write(
                            "@{0}{{{1},\n\tnote = {{{2} {3}}}\n}}\n".format(
                                bib["type"], k, args.wordforip, factors[f]
                            )
                        )
